...
def align_tops(top1, top2):
    # map (chain, residue_index, atom_name) -> atom_index
    d1 = {}
    for a in top1.atoms:
        if a.name != "CA":
            continue
        r = a.residue
        key = (r.chain.index, r.index, a.name)  # r.index 是 mdtraj internal residue index (0-based)
        d1[key] = a.index
    ...
    for a in top2.atoms:
        if a.name != "CA":
            continue
        r = a.residue
        key = (r.chain.index, r.index, a.name)
        if key in d1:
            mask1.append(d1[key])
            mask2.append(a.index)
...
def main(name):
    ...
    out = {}
    try:
        ref_aa = mdtraj.load(f'{args.atlas_dir}/chain-r1/{name}.pdb')
        topfile = f'{args.atlas_dir}/chain-r1/{name}.pdb'
        # traj_aa = mdtraj.load(f'{args.atlas_dir}/{name}.xtc', top=topfile)
        traj_aa = mdtraj.load(f'{args.atlas_dir}/chain-r1/{name}.xtc', top=topfile) \
            + mdtraj.load(f'{args.atlas_dir}/chain-r2/{name}.xtc', top=topfile) \
            + mdtraj.load(f'{args.atlas_dir}/chain-r3/{name}.xtc', top=topfile)
