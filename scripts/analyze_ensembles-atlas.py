...
def condense_sidechain_sasas(sasas, top):
    ...
    if top.n_atoms != sasas.shape[1]:
        raise ValueError(
            ...
            f"number of SASAs provided ({sasas.shape[1]})."
        )
...
def get_atom_indices_by_name(top, atom_name="CA"):
    return [a.index for a in top.atoms if a.name == atom_name]


def align_tops_by_ca_order(top1, top2):
    """
    只用 CA，按出現順序一一對齊。
    不看 residue number，不看 repr(a)。
    適合：
      - 兩邊都是同一段 protein
      - residue 編號可能不同
      - 但殘基順序一致
    """
    ca1 = get_atom_indices_by_name(top1, "CA")
    ca2 = get_atom_indices_by_name(top2, "CA")

    n_match = min(len(ca1), len(ca2))
    mask1 = ca1[:n_match]
    mask2 = ca2[:n_match]
...
def main(name):
    ...
    try:
        ...
        aftraj_aa = mdtraj.load(
            f'{args.pdbdir}/{name}.xtc',
            top=f'{args.pdbdir}/{name}.pdb'
        )
        ...
        if args.bb_only:
            print('Keeping only backbone atoms in AF')
            aftraj_aa.atom_slice(
                [a.index for a in aftraj_aa.top.atoms if a.name in ['CA', 'C', 'N', 'O', 'OXT']],
                True
            )
            print(f'AF has {aftraj_aa.n_atoms} atoms')

        elif args.ca_only:
            print('Keeping only CA atoms in AF')
            aftraj_aa.atom_slice([a.index for a in aftraj_aa.top.atoms if a.name == 'CA'], True)
            print(f'AF has {aftraj_aa.n_atoms} atoms')

        # 改這裡：只按 CA 絕對順序對齊
        refmask, afmask = align_tops_by_ca_order(traj_aa.top, aftraj_aa.top)

        if len(refmask) == 0 or len(afmask) == 0:
            raise ValueError(f"{name}: no CA atoms matched by order")

        print(f'Matched {len(refmask)} CA atoms by order')

        traj_aa.atom_slice(refmask, True)
        ref_aa.atom_slice(refmask, True)
        aftraj_aa.atom_slice(afmask, True)

        print(f'After alignment slice:')
        print(f'Reference has {traj_aa.n_atoms} atoms')
        print(f'Crystal has {ref_aa.n_atoms} atoms')
        print(f'AF has {aftraj_aa.n_atoms} atoms')
        ...
        if traj.n_atoms == 0 or aftraj.n_atoms == 0:
            raise ValueError(f"{name}: zero CA atoms left before PCA")
        ...
        try:
            out['emd_var'] = (
                np.trace(ref_covar + af_covar - 2 * sqrtm(ref_covar @ af_covar), axis1=1, axis2=2) ** 0.5
            ) * 10
        except Exception:
        ...
        out['ref_rms_pairwise_rmsd'] = np.square(
            get_rmsds(traj[RAND1].xyz, traj[RAND2].xyz, broadcast=True)
        ).mean() ** 0.5
        out['af_rms_pairwise_rmsd'] = np.square(
            get_rmsds(aftraj.xyz, aftraj.xyz, broadcast=True)
        ).mean() ** 0.5

        out['ref_self_mean_pairwise_rmsd'] = get_rmsds(traj[RAND1].xyz, traj[RAND1].xyz, broadcast=True).mean()
        out['ref_self_rms_pairwise_rmsd'] = np.square(
            get_rmsds(traj[RAND1].xyz, traj[RAND1].xyz, broadcast=True)
        ).mean() ** 0.5
        ...
        def get_emd(ref_coords1, ref_coords2, af_coords, seed_coords, K=None):
            ...
            emd = {}
            emd['ref|ref mean'] = (
                np.square(ref_coords1 - ref_coords1.mean(0)).sum(-1)
            ).mean() ** 0.5 / n_atoms ** 0.5 * 10
            ...
            emd['ref mean|ref2 mean'] = (
                np.square(ref_coords1.mean(0) - ref_coords2.mean(0)).sum() ** 0.5 / n_atoms ** 0.5 * 10
            )
            ...
            emd['ref mean|af mean'] = (
                np.square(ref_coords1.mean(0) - af_coords.mean(0)).sum() ** 0.5 / n_atoms ** 0.5 * 10
            )

            emd['ref|seed'] = (
                np.square(ref_coords1 - seed_coords).sum(-1)
            ).mean() ** 0.5 / n_atoms ** 0.5 * 10
            emd['ref mean|seed'] = (
                np.square(ref_coords1.mean(0) - seed_coords).sum(-1)
            ).mean() ** 0.5 / n_atoms ** 0.5 * 10

            emd['af|seed'] = (
                np.square(af_coords - seed_coords).sum(-1)
            ).mean() ** 0.5 / n_atoms ** 0.5 * 10
            emd['af|af mean'] = (
                np.square(af_coords - af_coords.mean(0)).sum(-1)
            ).mean() ** 0.5 / n_atoms ** 0.5 * 10
            emd['af mean|seed'] = (
                np.square(af_coords.mean(0) - seed_coords).sum(-1)
            ).mean() ** 0.5 / n_atoms ** 0.5 * 10

            return emd

        K = 2
        out['EMD,ref'] = get_emd(ref_coords[RAND1], ref_coords[RAND2], af_coords_ref_pca, seed_coords_ref_pca, K=K)
        out['EMD,af2'] = get_emd(ref_coords_af_pca[RAND1], ref_coords_af_pca[RAND2], af_coords, seed_coords_af_pca, K=K)
        out['EMD,joint'] = get_emd(ref_coords_joint_pca[RAND1], ref_coords_joint_pca[RAND2], af_coords_joint_pca, seed_coords_joint_pca, K=K)
...
if args.pdb_id:
    ...
    pdb_id = [nam.split('.')[0] for nam in os.listdir(args.pdbdir) if nam.endswith('.pdb')]
