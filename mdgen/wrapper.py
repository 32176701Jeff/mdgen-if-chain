...
class Wrapper(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        if self.args.warmup_steps > 0:
            ...
            start_ratio = self.args.warmup_start_ratio

            def lr_lambda(current_step):
                if current_step >= warmup_steps:
                    return 1.0
                # linear warmup
                return start_ratio + (1.0 - start_ratio) * (current_step / warmup_steps)
            ...
            return {
                ...
                "lr_scheduler": {
                    "scheduler": scheduler,
                    "interval": "step",   # IMPORTANT: step-based
                    "frequency": 1,
                },
            }
...
class NewMDGenWrapper(Wrapper):
    def __init__(self, args):
        ...
        #@@@@@@@@@@@fine tune
        if args.finetune:
            self._configure_finetune_requires_grad()
        #@@@@@@@@@@@fine tune
        ...
        if args.ema:
            ...
            self.cached_weights = None
    #@@@@@@@@@@@fine tune
    def _configure_finetune_requires_grad(self):
        """
        Configure requires_grad flags for fine-tuning stages.

        Controlled by args.unfreeze_start_layer:
        - -1 : only train chain_embedding (stage 1)
        -  k : unfreeze DiT layers from k to end (stage 2)
        -  0 : unfreeze all DiT layers (stage 3)
        """
        ...
        # --------------------------------------------------
        # 2. Always train chain embedding
        #    (dataset.py guarantees correct chain_ids)
        # --------------------------------------------------
        if getattr(self.args, 'use_chain_emb', False):
            if not hasattr(self.model, "chain_embedding"):
                raise AttributeError("args.use_chain_emb is True but LatentMDGenModel has no 'chain_embedding'")           
            for p in self.model.chain_embedding.parameters():
                p.requires_grad = True
        # --------------------------------------------------
        # 3. Handle DiT (Layers) unfreezing
        # --------------------------------------------------
        start_layer = self.args.unfreeze_start_layer
        if start_layer < 0:
            return
        if not hasattr(self.model, "layers"):
            raise AttributeError("LatentMDGenModel has no attribute 'layers'")
        dit_layers = self.model.layers
        n_layers = len(dit_layers)
        if start_layer >= n_layers:
            raise ValueError(
                f"unfreeze_start_layer={start_layer} >= number of DiT layers ({n_layers})"
            )
        # 解凍指定的 DiT Layers
        for i in range(start_layer, n_layers):
            for p in dit_layers[i].parameters():
                p.requires_grad = True
        # 同步解凍對應層數的 IPA Layers (若存在)
        if hasattr(self.model, "ipa_layers"):
            ipa_layers = self.model.ipa_layers
            n_ipa = len(ipa_layers)
            for i in range(min(start_layer, n_ipa-1), n_ipa):
                for p in ipa_layers[i].parameters():
                    p.requires_grad = True
    #@@@@@@@@@@@fine tune
    ...
    def prep_hyena_batch(self, batch):
        ...
        return {
            ...
            'model_kwargs': {
                ...
                'chain_ids': batch['chain_ids'], # @@@@@@@@@@@ chainID
