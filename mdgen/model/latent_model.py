...
class LatentMDGenModel(nn.Module):
    def __init__(self, args, latent_dim):
        ...
        # @@@@@@@@@@@ chain_emb
        if getattr(args, 'use_chain_emb', False):
            self.chain_embedding = nn.Embedding(4, args.embed_dim)
        # @@@@@@@@@@@ chain_emb
    ...
    def forward(self, x, t, mask,
        ...
        # @@@@@@@@@@@ chain_emb
        if getattr(self.args, 'use_chain_emb', False) and chain_ids is not None:
            if hasattr(self, 'chain_embedding'):
                x = x + self.chain_embedding(chain_ids)[:, None, :, :]
        # @@@@@@@@@@@ chain_emb
    ...
    def forward_inference(self, x, t, mask,
        ...
        if not self.args.design or self.args.dynamic_mpnn or self.args.mpnn:
            return self.forward(x, t, mask, start_frames, end_frames, x_cond, x_cond_mask, aatype, chain_ids)
        else:
            x_discrete = x[:, :, :, -20:]
            ...
            latent = self.forward(x, t, mask, start_frames, end_frames, x_cond, x_cond_mask, aatype, chain_ids)
