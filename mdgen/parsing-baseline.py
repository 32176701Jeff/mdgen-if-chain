...
def parse_train_args():
    ...
    group.add_argument('--atlas_gap', type=str)
    parser.add_argument(
        "--warmup_steps",
        type=int,
        default=0,
        help="Number of warmup steps for learning rate scheduling (0 = no warmup)."
    )
    parser.add_argument(
        "--warmup_start_ratio",
        type=float,
        default=0.1,
        help="Starting LR ratio for warmup (e.g., 0.1 means start from 0.1 * lr)."
    )
    parser.add_argument(
        "--unfreeze_start_layer",
        type=int,
        default=-1,
        help=(
            "Index of DiT layer to start unfreezing. "
            "-1 means do not unfreeze DiT layers (e.g., stage 1: chain embedding only). "
            "0 means unfreeze all DiT layers."
        )
    )
    #@@@@@@@@@@@@@ fine tune
    ...
    group.add_argument('--csv_dir', type=str, default=None, required=True)
    ...
    group.add_argument('--start', type=int, default=0,
                       help='Starting frame index for reading trajectory frames.')
    group.add_argument('--stride', type=int, default=1,
                       help='Stride for reading trajectory frames from the .npy file.')
    ...
    # if need chain_emb
    group.add_argument('--use_chain_emb', action='store_true', help="是否使用 chain embedding")
    #
